...
class JustWatch:
    api_base = 'https://api.justwatch.com'
    ...
    def __init__(self, country='AU', use_sessions=True, **kwargs):
        ...
        self.api_url_popular = '{api_base}/titles/{locale}/popular'.format(api_base=self.api_base,
                                                                           locale=self.locale)
        self.api_url_content = '{api_base}/content/'.format(api_base=self.api_base)
    ...
    def search_for_item(self, **kwargs):
        ...
        r = self.requests.post(self.api_url_popular, json=payload, headers=HEADER)
    ...
    def get_providers(self):
        providers = '{api_url_content}/providers/locale/{locale}'.format(api_url_content=self.api_url_content,
                                                                         locale=self.locale)
        r = self.requests.get(providers, headers=HEADER)
    ...
    def get_genres(self):
        genres = '{api_url_content}/genres/locale/{locale}'.format(api_url_content=self.api_url_content,
                                                                   locale=self.locale)
        r = self.requests.get(genres, headers=HEADER)
    ...
    def get_title(self, title_id, content_type='movie'):
        titles = '{api_url_content}/titles/{content_type}/{title_id}/locale/{locale}'
        titles = titles.format(api_url_content=self.api_url_content,
                               content_type=content_type,
                               title_id=title_id,
                               locale=self.locale)

        r = self.requests.get(titles, headers=HEADER)
